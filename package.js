var mdcVersion = '0.2.0';
var revision = 0;
var mdcDistPath = 'dist'; // This relies on the symlink.
var mdcSrcPath = 'src'; // This relies on the symlink.

var deps = {
  'Meteor': '1.4.2',
  'SCSS': 'fourseven:scss@3.13.0'
};

Npm.depends({
  'material-components-web': mdcVersion
});

Package.describe({
  name: 'zodiase:mdc-assets',
  version: (revision > 0) ? mdcVersion + '_' + revision : mdcVersion,
  summary: 'Assets of Material Components Web. Not intended to be used directly.',
...
Package.onUse(function(api) {
  ...
  api.addAssets(prepandPathToFiles(distAssets, mdcDistPath), 'server');

  // Add sass files to be imported by users.
  // Has to be added to `client` platform or `fourseven:scss` won't see them.
  api.addFiles(prepandPathToFiles(sassFiles, mdcSrcPath), 'client', {isImport: true});

  // Add other mdl assets.
  api.addAssets(prepandPathToFiles(miscAssets, mdcSrcPath), ['client', 'server']);

  api.addFiles('serve.js', 'server');
  api.export(['_mdcGetAsset'], 'server');
});

Package.onTest(function (api) {
  api.use('zodiase:mdc-assets');
});
...
var miscAssets = [];
///>>>>MISCASSETS
//miscAssets = ["images/buffer.svg","images/tick-mask.svg","images/tick.svg"];
///<<<<MISCASSETS
